...
from codedocai.llm.docstring_builder import build_docstring_prompt
from codedocai.mutator.source_writer import inject_docstring
...
from codedocai.semantic.ir_export import export_ir, file_hash, load_ir
from codedocai.graph.exporters import export_call_graph_json, export_ir_csv
...
def run_pipeline(config: AppConfig) -> None:
    ...
    def _process_single(file_ir):
        ...
        from codedocai.semantic.hallucination_check import build_symbol_whitelist, check_summary
    ...
    def _process_batch(batch_files):
        nonlocal hallucination_flags
        from codedocai.semantic.hallucination_check import build_symbol_whitelist
        batch_data = [(f, build_symbol_whitelist(f)) for f in batch_files if not f.summary]
        if not batch_data: return
        
        from codedocai.llm.prompt_builder import build_batch_summary_prompt
    ...
    with Progress(SpinnerColumn(), TextColumn("{task.description}"), console=console) as progress:
        ...
        with ThreadPoolExecutor(max_workers=config.concurrency) as executor:
            ...
            for future in as_completed(futures):
                # Approximate progress: for simplicity, we advance by 1 for each future
                # In batches of 5, this is slightly inaccurate visually but logically sound
                progress.advance(task)
