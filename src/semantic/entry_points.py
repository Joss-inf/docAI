...
from src.semantic.ir_schema import ModuleRole, ProjectIR
...
def detect_entry_points(project: ProjectIR, call_graph: CallGraph) -> list[EntryPoint]:
    ...
    for file_ir in project.files:
        for func in file_ir.functions:
            ...
            if ep_type:
                ep = EntryPoint(file_path=file_ir.file_path, function_id=node_id, type=ep_type)
                entry_points.append(ep)

    # Trace reachability via Call Graph
    for ep in entry_points:
        if ep.function_id in call_graph.nx_graph:
